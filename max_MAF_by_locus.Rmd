...
original script: allele_frequency_parse3.mb.py
```{python}
...
#produce whitelist of max_MAF_SNP by locus
```{r}
frequency_data = read.csv("I:/WAE_RAD_Data/novoseq2/STACKS/Filter_steps/genomics/v6_novoseq2_allele_freq_info.txt", sep = "\t")

max_maf_by_locus = frequency_data %>% 
  group_by(Chrom) %>% 
  top_n(1)

output.file <- file("I:/WAE_RAD_Data/novoseq2/STACKS/Filter_steps/genomics/max_MAF_SNP_by_locus_novoseq2.txt", "wb")
write.table(max_maf_by_locus[,1:2], file = output.file, row.names = F, quote = F, col.names = F, sep = "\t")
close(output.file)
```
